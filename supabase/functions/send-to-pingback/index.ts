...
serve(async (req) => {
  ...
  try {
    ...
    const pingbackResponse = await fetch(await resolvePingbackUrl("default"), {
      method: "POST",
    ...
    if (!pingbackResponse.ok) {
      console.error("Pingback webhook failed:", pingbackResponse.status);
      return new Response(
    ...
    console.error("send-to-pingback error:", error);
