...
interface HeroSectionEventoProps {
  onOpenModal: () => void;
}
...
export function HeroSectionEvento({ onOpenModal }: HeroSectionEventoProps) {
  return (
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full">
        ...
        <div className="text-left max-w-[650px]">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-[3.4rem] font-bold leading-[1.08] tracking-tight mb-4 animate-slide-up">
            Sua empresa{" "}
            <span className="text-gradient">não será mais como é hoje.</span>
            {" "}A pergunta é:{" "}
            <span className="text-primary">quem vai mudá-la?</span>
          </h1>

          <p className="text-lg md:text-xl text-muted-foreground mb-6">
            Você — ou o seu concorrente?
          </p>

          {/* Sub */}
          <p className="text-sm md:text-base text-muted-foreground leading-relaxed max-w-[600px] mb-8">
            Uma imersão de empresário para empresário.
            <br />
          ...
          {/* Event info bar */}
          <div className="inline-flex items-center gap-4 md:gap-6 px-5 py-3 rounded-lg bg-card/60 border border-border/40 backdrop-blur-sm mb-8">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Calendar className="w-4 h-4 text-accent" />
              <span className="font-semibold text-foreground">24 e 25/FEV</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
