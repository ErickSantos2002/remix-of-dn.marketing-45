...
interface HeroSectionModernGratuitoProps {
  onOpenModal: () => void;
}

export function HeroSectionModernGratuito({ onOpenModal }: HeroSectionModernGratuitoProps) {
  ...
  return (
    <section id="inicio" className="relative min-h-[100dvh] lg:min-h-screen flex items-start overflow-hidden">
      ...
      <div 
        className="absolute top-0 left-0 w-[70%] h-[70%] -translate-x-1/4 -translate-y-1/4 opacity-50 pointer-events-none z-[2] glow-effect-primary"
        style={{
          background: 'radial-gradient(ellipse at center, hsl(var(--primary) / 0.25) 0%, transparent 60%)',
        }}
      />
      <div 
        className="absolute bottom-0 right-0 w-[60%] h-[60%] translate-x-1/4 translate-y-1/4 opacity-40 pointer-events-none z-[2] glow-effect-secondary"
        style={{
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-8 max-w-7xl mx-auto">
          ...
          <div className="w-full lg:w-[55%] xl:w-[50%] text-center lg:text-left">
            ...
            <div className="flex flex-col items-center lg:items-start gap-3">
              {/* Free badge above CTA */}
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-success/30 bg-success/5">
                <Check className="w-4 h-4 text-success" />
                <span className="text-sm font-medium text-success">EVENTO GRATUITO</span>
              </span>
          ...
          <div className="hidden sm:flex w-full lg:w-[45%] xl:w-[45%] items-center justify-center">
